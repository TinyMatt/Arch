...
public sealed class ConservativeCommandBuffer : IDisposable
{
    ...
    /// <summary>
    ///     Gets the amount of <see cref="Entity"/> instances targeted by this <see cref="ConservativeCommandBuffer"/>.
    /// </summary>
    public int Size { get; private set; }

    /// <summary>
    ///     All <see cref="Entity"/>'s created or modified in this <see cref="ConservativeCommandBuffer"/>.
    /// </summary>
    internal PooledList<Entity> Entities { get; set; }

    /// <summary>
    ///     A map that stores some additional information for each <see cref="Entity"/>, which is needed for the internal <see cref="ConservativeCommandBuffer"/> operations.
    /// </summary>
    internal PooledDictionary<int, BufferedEntityInfo> BufferedEntityInfo { get; set; }

    /// <summary>
    ///     All create commands recorded in this <see cref="ConservativeCommandBuffer"/>. Used to create <see cref="Entity"/>'s during <see cref="Playback"/>.
    /// </summary>
    ...
    /// <summary>
    ///     Registers a new <see cref="Entity"/> into the <see cref="ConservativeCommandBuffer"/>.
    ///     An <see langword="out"/> parameter contains its <see cref="Arch.ConservativeCommandBuffer.BufferedEntityInfo"/>.
    /// </summary>
    /// <param name="entity">The <see cref="Entity"/> to register.</param>
    /// <param name="info">Its <see cref="BufferedEntityInfo"/> which stores indexes used for <see cref="ConservativeCommandBuffer"/> operations.</param>
    ...
    public void Playback()
    {
        ...
        for (var index = 0; index < Adds.Count; index++)
        {
            ...
            Debug.Assert(World.IsAlive(entity), $"ConservativeCommandBuffer can not to add components to the dead {wrappedEntity.Entity}");
        ...
        for (var index = 0; index < Sets.Count; index++)
        {
            ...
            Debug.Assert(World.IsAlive(entity), $"ConservativeCommandBuffer can not to set components to the dead {wrappedEntity.Entity}");
        ...
        for (var index = 0; index < Removes.Count; index++)
        {
            ...
            var entity = Resolve(wrappedEntity.Entity);
            Debug.Assert(World.IsAlive(entity), $"ConservativeCommandBuffer can not to remove components from the dead {wrappedEntity.Entity}");
    ...
    /// <summary>
    ///     Disposes the <see cref="ConservativeCommandBuffer"/>.
    /// </summary>
    public void Dispose()
    {
        Entities.Dispose();
